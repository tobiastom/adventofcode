...
typealias Rules = [String: [String: Int]]

func rulesFromInput(input: String) -> Rules {
    var rules = Rules()

    input.components(separatedBy: "\n").forEach { line in
        let components = line.components(separatedBy: "bags contain").map {
            $0.trimmingCharacters(in: .whitespaces)
        }

        guard let name = components.first, let contains = components.last else {
            return
        }

        rules[name] = [:]
        if contains.hasPrefix("no") {
            return
        }

        contains.components(separatedBy: ", ").forEach { bag in
            let split = bag.split(separator: " ")
            guard let sizeAsString = split.first, let size = Int(sizeAsString) else {
                return
            }

            let subBag = split[1...2].joined(separator: " ")
            rules[name]![subBag] = size
        }
    }
...
func containsShinyGold(key: String, _ rules: Rules) -> Bool {
    guard let children = rules[key] else {
        return false
    }

    if children.keys.contains("shiny gold") {
        return true
    }

    return children.keys.first(
        where: { containsShinyGold(key: $0, rules) }) != nil
}

func countBags(key: String, _ rules: Rules) -> Int {
    guard let bags = rules[key] else {
        return 0
    }

    let bagCount = rules[key]?.values.reduce(0, +) ?? 0
    return bags.reduce(bagCount) {
        $0 + (countBags(key: $1.key, rules) * $1.value)
    }
}

var rules = rulesFromInput(input: input)
print(rules.keys.filter { containsShinyGold(key: $0, rules) }.count)
print(countBags(key: "shiny gold", rules))
