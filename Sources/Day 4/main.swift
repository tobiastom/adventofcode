...
//  File.swift
//
//  Created by Tobias Tom on 04.12.20.
...
let passportData = input.components(separatedBy: "\n\n").map { pairs in
    return pairs.components(separatedBy: .whitespacesAndNewlines)
        .reduce(into: [String: String](), { passport, item in
            let pair = item.split(separator: ":", maxSplits: 2)
            passport[String(pair.first!)] = String(pair.last!)
        })
}
...
struct Passport {
    ...
    enum Height {
        ...
        init?(height: String?) {
            ...
            if height.hasSuffix("cm") {
                self = .cm(Int(height.dropLast(2)) ?? 0)
            } else if height.hasSuffix("in") {
                self = .inch(Int(height.dropLast(2)) ?? 0)
            } else {
                return nil
            }
        }
    ...
    init?(details: [String: String]) {
        guard let byr = Int(details["byr"] ?? ""), 1920...2002 ~= byr else {
            return nil
        }

        guard let iyr = Int(details["iyr"] ?? ""), 2010...2020 ~= iyr else {
            return nil
        }

        guard let eyr = Int(details["eyr"] ?? ""), 2020...2030 ~= eyr else {
            return nil
        }

        guard let hgt = Height(height: details["hgt"]) else {
            return nil
        }

        switch hgt {
            case .cm(let cm):
                guard (150...193).contains(cm) else {
                    return nil
                }
            case .inch(let inch):
                guard (59...76).contains(inch) else {
                    return nil
                }
        }

        guard let hcl = details["hcl"],
              hcl.hasPrefix("#"),
              hcl.count == 7,
              hcl.dropFirst().allSatisfy(\.isHexDigit)
        else {
            return nil
        }

        guard let ecl = EyeColor(rawValue: details["ecl"] ?? "") else {
            return nil
        }

        guard let pid = details["pid"], pid.count == 9, pid.allSatisfy(\.isNumber) else {
            return nil
        }
        ...
        eyeColor = ecl
        passportID = pid
        countryID = details["cid"]
    }
}

print(passportData.filter { requiredKeys.allSatisfy($0.keys.contains) }.count)

print(passportData.map { details in
    return Passport(details: details)
}.compactMap { $0 }.count)
