...
//  main.swift
//
//  Created by Tobias Tom on 02.12.20.
...
struct Line {
    let requirement: ClosedRange<Int>
    let letter: Character
    let password: String

    static func lineFromInput(_ string: String) -> Line {
        let parts = string.components(separatedBy: " ")
        let bounds = parts[0].components(separatedBy: "-")
        let lowerBound = Int(bounds.first ?? "")!
        let upperBound = Int(bounds.last ?? "")!

        return Line(
            requirement: lowerBound...upperBound,
            letter: parts[1].first ?? " ",
            password: parts[2]
        )
    }

    var isValidSledRentalPlace : Bool {
        requirement.contains(
            password.filter { $0 == letter }.count
        )
    }

    var isValidOfficialTobogganCorporateAuthenticationSystem: Bool {
        let password = Array(self.password)
        let lowerChar = password[requirement.lowerBound - 1]
        let upperChar = password[requirement.upperBound - 1]

        return (lowerChar == letter) != (upperChar == letter)
    }
}

func countMatches(_ input: String, _ check: (Line) -> Bool) -> Int {
    let result = input.split(whereSeparator: \.isNewline).map { input -> Bool in
        let line = Line.lineFromInput(String(input))
        return check(line)
    }
...
print(countMatches(input, { $0.isValidSledRentalPlace }))
print(countMatches(input, { $0.isValidOfficialTobogganCorporateAuthenticationSystem }))
